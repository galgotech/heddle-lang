...
func (o *InteractiveOrchestrator) OrchestrateTask(ctx context.Context, task models.Task) {
	logger.L().Debug("workflow execution initiated: starting interactive task orchestration",
	...
	if task.ClientID != "" {
		...
		logger.L().Warn("workflow client warning: no client ID specified, running in headless mode",
			logger.Component("control-plane"),
			logger.TraceID(task.ID),
		)
	}
	...
	var clientStream *registry.NodeStream
	if task.ClientID != "" {
		var ok bool
		clientStream, ok = o.registry.GetNode(task.ClientID)
		if !ok {
			logger.L().Warn("client stream warning: stream not found or client not registered, running in headless mode",
				logger.Component("control-plane"),
				logger.TraceID(task.ID),
				logger.ClientID(task.ClientID),
			)
	...
	for _, flowID := range program.Workflows {
		...
		logger.L().Info("workflow execution started: executing flow interactively",
			logger.Component("control-plane"),
			logger.TraceID(task.ID),
...
func (o *InteractiveOrchestrator) executeStepInteractive(ctx context.Context, workflowID string, prog ir.Program, stepID string, prevTaskIDs []string, schemas map[string]schema.StepSchemas, clientStream transport.ExchangeStream) error {
	...
	logger.L().Info("approval request: prompting approval for step",
		logger.Component("control-plane"),
		logger.TraceID(workflowID),
		logger.TaskID(stepID),
	...
	if clientStream != nil {
		logger.L().Debug("approval request: sending interactive prompt to client",
			logger.Component("control-plane"),
			logger.TraceID(workflowID),
			logger.TaskID(stepID),
			logger.Capability(capability),
		...
		if err != nil {
			err = fmt.Errorf("failed to send interactive prompt to client: %w", err)
			logger.L().Error("approval request failed: failed to send prompt to client stream",
				logger.Component("control-plane"),
				logger.TraceID(workflowID),
				logger.TaskID(stepID),
				logger.Error(err),
		...
		logger.L().Debug("approval response: awaiting decision from client stream",
			logger.Component("control-plane"),
			logger.TraceID(workflowID),
			logger.TaskID(stepID),
		)
		...
		if err != nil {
			err = fmt.Errorf("failed to receive interactive response from client: %w", err)
			logger.L().Error("approval response failed: error receiving decision from client stream",
				logger.Component("control-plane"),
				logger.TraceID(workflowID),
				logger.TaskID(stepID),
				logger.Error(err),
		...
		if string(msg.DataBody) != "APPROVE" {
			err = fmt.Errorf("step execution rejected by user")
			logger.L().Warn("approval response: step execution rejected by user",
				logger.Component("control-plane"),
				logger.TraceID(workflowID),
				logger.TaskID(stepID),
			)
		...
		logger.L().Info("approval response: step execution approved by client",
			logger.Component("control-plane"),
			logger.TraceID(workflowID),
			logger.TaskID(stepID),
		)
		...
		logger.L().Debug("approval simulation: awaiting simulated interactive step approval",
			logger.Component("control-plane"),
			logger.TraceID(workflowID),
			logger.TaskID(stepID),
		)
		...
		case <-ctx.Done():
			err := ctx.Err()
			logger.L().Error("approval simulation failed: context cancelled during headless approval simulation",
				logger.Component("control-plane"),
				logger.TraceID(workflowID),
				logger.TaskID(stepID),
				logger.Error(err),
		...
		case <-time.After(10 * time.Millisecond):
			logger.L().Info("approval simulation: step approved for execution in headless mode",
				logger.Component("control-plane"),
				logger.TraceID(workflowID),
				logger.TaskID(stepID),
			)
