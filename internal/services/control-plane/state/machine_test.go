...
func TestStateMachine_Transition(t *testing.T) {
	...
	assert.Equal(t, Running, retrieved.GetState())
	...
	retrieved, _ = sm.GetNode("node-1")
	assert.Equal(t, Completed, retrieved.GetState())
	...
	assert.Equal(t, Failed, retrieved2.GetState())
	assert.Equal(t, testErr, retrieved2.GetError())
}
