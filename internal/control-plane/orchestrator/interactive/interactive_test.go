...
func TestInteractiveOrchestrator_OrchestrateTask_Success(t *testing.T) {
	s := newMockServer()
	...
	o := interactive.NewInteractiveOrchestrator(reg)
	...
	assert.Equal(t, "task-interactive-1", s.signaledTaskID)
